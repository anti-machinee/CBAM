...
import sys
sys.path.append(".")
from models.modules.cbam.cbam import CBAM
...
class BottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels, expansion=4, stride=1, use_cbam=True):
        ...
        # only the first conv will be affected by the given stride parameter. The rest have default stride value (
        # which is 1).
        ...
        # since the input has to be same size with the output during the identity mapping, whenever the stride or the
        # number of output channels are more than 1 and expansion*out_channels respectively, the input, x,
        # has to be downsampled to the same level as well.
        self.identity_connection = nn.Sequential()
        if stride != 1 or in_channels != expansion * out_channels:
            self.identity_connection = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=expansion * out_channels, kernel_size=1, stride=stride,
                          bias=False),
...
class ResNet50(nn.Module):
    def __init__(self, use_cbam=True, image_depth=3, num_classes=6):
        ...
        self.conv_block1 = nn.Sequential(
            nn.Conv2d(kernel_size=7, stride=2, in_channels=image_depth, out_channels=self.in_channels, padding=3,
                      bias=False),
    ...
    def make_layer(self, out_channels, num_blocks, stride, use_cbam):
        ...
        for stride in strides:
            layers.append(BottleNeck(in_channels=self.in_channels, out_channels=out_channels, stride=stride,
                                     expansion=self.expansion, use_cbam=use_cbam))
...
def main():
    ...
    _input = torch.ones((7, 3, 224, 224))
    network = ResNet50(num_classes=10)
    y1, y2 = network(_input)
    print("y1", y1.shape)
    print("y1", y2.shape)
